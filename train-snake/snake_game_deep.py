...
def evalGenomes(population, generations, survival_fraction=0.1, fitness_threshold=200, mutation_rate=0.03, mutation_range=[-2,2], nn_shape=[24,30,30,4], activation_functions=['tanh','tanh','tanh','softmax'], initial_config=False, watch=False):
    '''
    ...
    if not initial_config:
        ...
        child_nets=mutate(nets[int(len(nets)*survival_fraction):],mutation_range=mutation_range, mutation_rate=mutation_rate, nn_shape=nn_shape, activation_functions=activation_functions)
...
def mutate(nets, mutation_range=[-2,2], mutation_rate=0.03, nn_shape=[24,30,30,4], activation_functions=['tanh','tanh','tanh','softmax']):
    mutated_nets=[]
...
def crossover(agents,nn_shape,activation_functions):
    ...
    for i in range(int(population-0.5*len(agents))):
